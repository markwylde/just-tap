...
const runTest = async (context, test) => {
  ...
  const cleanup = await test.fn({
    ...
    throws: (fn, expected, message) => {
      ...
      const ok = deepEqual(error, expected, { strict: true });
      const defaultMessage = 'expected error to be thrown';
      if (ok) {
        local.ok(message || defaultMessage);
      } else {
        local.notOk((message ? message + ' - ' : '') + defaultMessage);
        logComment(diffObjects(error, expected));
      }
    },
    notThrows: (fn, message) => {
      ...
      const ok = !error;
      const defaultMessage = 'expected error to not be thrown';
      if (ok) {
        local.ok(message || defaultMessage);
      } else {
        local.notOk((message ? message + ' - ' : '') + defaultMessage);
