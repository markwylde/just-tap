...
const runTest = async (context, test) => {
  ...
  await test.fn({
    ...
    deepEqual: (a, b, message) => {
      ...
      const defaultMessage = `expected ${stringA} to deep equal ${stringB}`;
      if (deepEqual(stringA, stringB)) {
        local.ok(message || defaultMessage);
      } else {
        local.notOk((message ? message + ' - ' : '') + defaultMessage);
      }
    ...
    notDeepEqual: (a, b, message) => {
      const stringA = JSON.stringify(a);
      const stringB = JSON.stringify(b);
      const defaultMessage = `expected ${stringA} to not deep equal ${stringB}`;
      if (!deepEqual(stringA, stringB)) {
        local.ok(message || defaultMessage);
      } else {
        local.notOk((message ? message + ' - ' : '') + defaultMessage);
